...
import { Loader2, AlertCircle, CheckCircle2, Zap, Wrench, Search } from "lucide-react";
...
export default function PdfValidationPanel() {
  ...
  const handleCompare = async () => {
    if (!clientPdf || !outputPdf) {
      setError("Please upload both PDF files before comparing.");
      return;
    }
    setError(null);
    setResult(null);
    ...
    try {
      ...
      if (err instanceof TypeError && err.message.includes("fetch")) {
        setError("Cannot reach backend server at " + API_BASE + ". Please ensure the API is running.");
      } else {
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card p-6 space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <FileUploadZone
            label="Upload Client Test Script (PDF)"
            file={clientPdf}
            onFile={setClientPdf}
          />
          <FileUploadZone
            label="Upload V-Assure Output (PDF)"
            file={outputPdf}
            onFile={setOutputPdf}
          />
        </div>
              ...
              <div className="space-y-4">
                <div className="diff-red rounded-lg px-5 py-4 flex items-center gap-3">
                  ...
                  <p className="text-sm font-semibold text-red-300">
                    Found {result.summary.total_issues} issue(s)
                  </p>
                </div>
          ...
          {result.setup_differences && Object.keys(result.setup_differences).length > 0 && (
            <section className="space-y-3">
              <h3 className="text-base font-bold text-foreground flex items-center gap-2">
                ...
                Setup Steps Differences
              </h3>
              {Object.keys(result.setup_differences)
                .sort((a, b) => parseInt(a) - parseInt(b))
                .map((step) => (
                  <SetupStepCard
                    key={step}
                    stepNum={step}
                    diffs={result.setup_differences[step]}
                  />
                ))}
            </section>
          )}
          ...
          {result.execution_differences && Object.keys(result.execution_differences).length > 0 && (
            <section className="space-y-3">
              <h3 className="text-base font-bold text-foreground flex items-center gap-2">
                ...
                Execution Steps Differences
              </h3>
              {Object.keys(result.execution_differences)
                .sort((a, b) => parseInt(a) - parseInt(b))
                .map((step) => (
                  <ExecutionStepCard
                    key={step}
                    stepNum={step}
                    diffs={result.execution_differences[step]}
                  />
                ))}
            </section>
          )}
